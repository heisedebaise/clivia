...
public class UserCtrl {
    ...
    public Object signUp() {
        userService.signUp(request.get("uid"), request.get("password"), request.get("type"), request.get("inviter"),
                request.get("grade"));
    ...
    @Execute(name = "password", permit = "0", validates = {
            ...
            @Validate(validator = Validators.EQUALS, parameters = {"new",
                    "repeat"}, failureCode = 16, failureArgKeys = {UserModel.NAME + ".password.repeat",
                    UserModel.NAME + ".password.new"}),
            @Validate(validator = UserService.VALIDATOR_SIGN)})
    ...
    public Object gesture() {
        return userService.gesture(request.get("old"), request.get("new")) ? ""
                : templates.get().failure(151018, message.get(UserModel.NAME + ".gesture.illegal"), null, null);
    }
    ...
    public Object query() {
        return userService.query(request.get("uid"), request.get("idcard"), request.get("name"), request.get("nick"),
                request.get("mobile"), request.get("email"), request.get("weixin"), request.get("qq"),
                request.get("code"), request.getAsInt("minGrade", -1), request.getAsInt("maxGrade", -1),
    ...
    public Object create() {
        userService.create(request.get("uid"), request.get("password"), request.get("idcard"), request.get("name"),
                request.get("nick"), request.get("mobile"), request.get("email"), request.get("weixin"),
                request.get("qq"), request.get("avatar"), request.getAsInt("gender"), request.getAsSqlDate("birthday"),
                userService.id(), request.getAsInt("grade"), request.getAsInt("state"));

        return "";
    }

    @Execute(name = "update", validates = {@Validate(validator = Validators.ID, parameter = "id", failureCode = 22),
            @Validate(validator = Validators.MAX_LENGTH, number = {100}, parameter = "idcard", failureCode = 7),
            @Validate(validator = Validators.MAX_LENGTH, number = {100}, parameter = "name", failureCode = 8),
            @Validate(validator = Validators.MAX_LENGTH, number = {100}, parameter = "nick", failureCode = 9),
            @Validate(validator = Validators.MOBILE, emptyable = true, parameter = "mobile", failureCode = 10),
    ...
    @Execute(name = "delete", validates = {@Validate(validator = Validators.ID, parameter = "id", failureCode = 22),
            @Validate(validator = Validators.SIGN),
            @Validate(validator = UserService.VALIDATOR_EXISTS, parameter = "id", failureCode = 25)})
