 ...
 * @author lpw
 */
@Service(AccountTypeSupport.NAME + AccountTypes.REFUND)
public class RefundImpl extends AccountTypeSupport implements AccountType {
    @Override
    public String getName() {
        return AccountTypes.REFUND;
    }
    ...
    public boolean complete(AccountModel account, LogModel log) {
        ...
        if (log.getState() == LogService.State.Pass.ordinal()) {
            ...
            account.setRefund(account.getRefund() + log.getAmount());
